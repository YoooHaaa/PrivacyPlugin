...
void *ElfImg::GetModuleBase(const char *name) {
    ...
    while (fgets(buff, sizeof(buff), maps)) {
        //if ((strstr(buff, "r--p") || strstr(buff, "r-xp")) && strstr(buff, name)) {

        if (strstr(buff, name)){
            if (strstr(buff, "r--p") || strstr(buff, "r-xp")){
                found = 1;
        ...
        if (strstr(buff, "r-xp") && strstr(buff, name)) {
            found = 1;
            __android_log_print(ANDROID_LOG_DEBUG, "dlopen", "%s\n", buff);
            break;
        }
    ...
    if (!found) {
        LOGE("failed to read load address 1 for %s", name);
        return nullptr;
    }

    if (sscanf(buff, "%lx", &load_addr) != 1)
        LOGE("failed to read load address 2 for %s", name);
